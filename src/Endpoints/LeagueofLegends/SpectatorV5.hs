...
module Endpoints.LeagueofLegends.SpectatorV5 where
...
activeGamesBySummoner :: Platform -> EncryptedPUUID -> IO (TypedRequest CurrentGameInfo)
activeGamesBySummoner platform encryptedPUUID = do
  request <- parseRequest $ platformToUrl platform ++ "/lol/spectator/v5/active-games/by-summoner/" ++ encryptedPUUID
  return $ TypedRequest request

featuredGames :: Platform -> IO (TypedRequest FeaturedGames)
featuredGames platform = do
  request <- parseRequest $ platformToUrl platform ++ "/lol/spectator/v5/featured-games"
  return $ TypedRequest request
