...
module Endpoints.VALORANT.ValMatchV1 where
...
matches platform matchId = do
  request <- parseRequest $ platformToUrl platform ++ "/val/match/v1/matches/" ++ matchId
  return $ TypedRequest request

matchlistsByPuuid :: Platform -> Puuid -> IO (TypedRequest MatchlistDto)
matchlistsByPuuid platform puuid = do
  request <- parseRequest $ platformToUrl platform ++ "/val/match/v1/matchlists/by-puuid/" ++ puuid
  return $ TypedRequest request

recentMatchesByQueue :: Platform -> Queue -> IO (TypedRequest RecentMatchesDto)
recentMatchesByQueue platform queue = do
  request <- parseRequest $ platformToUrl platform ++ "/val/match/v1/recent-matches/by-queue/" ++ queue
  return $ TypedRequest request
